...
const RESEARCH_AREAS = [
  ...
  {
    icon: "assets/icons/Complier.png",
    title: { en: "Compilers", ko: "컴파일러", ja: "コンパイラ", es: "Compiladores" },
    description: {
      en: "Compiler research spanning AI/ML compilers (TVM/XLA/MLIR-style graph optimization), hardware-aware compilation for FPGA/GPU/NPU backends, quantum circuit compilation for NISQ devices, and classical compiler and programming-language theory built on LLVM.",
      ko: "AI/ML 컴파일러(TVM/XLA/MLIR 스타일 그래프 최적화), FPGA/GPU/NPU 대상 하드웨어 인식 컴파일, NISQ 장치를 위한 양자 회로 컴파일, LLVM 기반의 고전 컴파일러 및 프로그래밍 언어 이론을 아우르는 컴파일러 연구.",
      ja: "AI/MLコンパイラ（TVM/XLA/MLIR系のグラフ最適化）、FPGA/GPU/NPU向けハードウェア対応コンパイル、NISQデバイス向けの量子回路コンパイル、LLVMベースの古典的コンパイラ・プログラミング言語理論にまたがるコンパイラ研究。",
      es: "Investigación en compiladores que abarca compiladores de IA/ML (optimización de grafos al estilo TVM/XLA/MLIR), compilación consciente del hardware para FPGA/GPU/NPU, compilación de circuitos cuánticos para dispositivos NISQ y teoría clásica de compiladores y lenguajes de programación basada en LLVM."
    },
    detailDescription: {
      en: "Compiler research forms the connective tissue between high-level program intent and the physical hardware that executes it, and our work spans four complementary directions. First, AI/ML compilation: we build graph-level optimization passes for deep learning models in the spirit of TVM, XLA, and MLIR — operator fusion, layout transformation, kernel scheduling, and autotuning that extract maximum throughput from modern accelerators. Second, hardware-aware compilation: lowering computational graphs onto FPGAs, GPUs, and emerging NPUs, with explicit modeling of memory hierarchies, dataflow patterns, and backend-specific constraints to deliver low-latency inference on edge devices. Third, quantum compilation: circuit optimization, gate decomposition, qubit routing, and noise-aware transpilation that adapt logical quantum circuits to the connectivity and error profiles of real NISQ hardware. Finally, classical compiler and programming language theory: leveraging LLVM-based infrastructure for static analysis and program transformations, and exploring formal semantics that guarantee correctness of optimizations. This thread underpins many of our other research areas — from AI semiconductors and quantum computing to medical AI deployment on edge devices.",
      ko: "컴파일러 연구는 고수준 프로그램 의도와 그것을 실행하는 물리적 하드웨어를 잇는 결합 조직이며, 본 연구는 상호 보완적인 네 방향으로 진행됩니다. 첫째, AI/ML 컴파일: TVM, XLA, MLIR의 정신을 따라 딥러닝 모델을 위한 그래프 수준의 최적화 패스를 구축합니다 — 연산자 융합, 레이아웃 변환, 커널 스케줄링, 그리고 최신 가속기에서 최대 처리량을 끌어내는 자동 튜닝. 둘째, 하드웨어 인식 컴파일: 계산 그래프를 FPGA, GPU, 신흥 NPU로 내리되 메모리 계층, 데이터 흐름, 백엔드별 제약을 명시적으로 모델링하여 엣지 장치에서 저지연 추론을 제공합니다. 셋째, 양자 컴파일: 회로 최적화, 게이트 분해, 큐비트 라우팅, 실제 NISQ 하드웨어의 연결성과 오류 프로파일에 적응하는 노이즈 인식 트랜스파일레이션. 마지막으로, 고전 컴파일러 및 프로그래밍 언어 이론: LLVM 기반 인프라를 활용한 정적 분석과 프로그램 변환, 그리고 최적화의 정확성을 보장하는 형식 의미론. 이 흐름은 AI 반도체, 양자 컴퓨팅, 엣지 의료 AI 배포 등 다른 연구 영역의 토대가 됩니다.",
      ja: "コンパイラ研究は、高水準のプログラム意図とそれを実行する物理ハードウェアをつなぐ結合組織であり、本研究は相補的な4つの方向で展開します。第一に、AI/MLコンパイル：TVM、XLA、MLIRの精神に倣い、深層学習モデル向けのグラフレベル最適化パスを構築します — 演算子融合、レイアウト変換、カーネルスケジューリング、そして現代のアクセラレータから最大スループットを引き出すオートチューニング。第二に、ハードウェア対応コンパイル：計算グラフをFPGA、GPU、新興NPUへ落とし込み、メモリ階層、データフロー、バックエンド固有の制約を明示的にモデル化してエッジデバイスでの低遅延推論を実現します。第三に、量子コンパイル：回路最適化、ゲート分解、キュービットルーティング、実NISQハードウェアの接続性とエラープロファイルに適応するノイズ対応トランスパイル。最後に、古典コンパイラとプログラミング言語理論：LLVMベースの基盤を活用した静的解析とプログラム変換、最適化の正当性を保証する形式意味論。この研究軸はAI半導体、量子コンピューティング、エッジ向け医療AIデプロイなど他の研究領域の基盤となります。",
      es: "La investigación en compiladores es el tejido conectivo entre la intención del programa de alto nivel y el hardware físico que lo ejecuta, y nuestro trabajo abarca cuatro direcciones complementarias. Primero, compilación de IA/ML: construimos pasadas de optimización a nivel de grafo para modelos de aprendizaje profundo al estilo de TVM, XLA y MLIR — fusión de operadores, transformación de disposiciones, planificación de kernels y autoajuste para extraer el máximo rendimiento de los aceleradores modernos. Segundo, compilación consciente del hardware: bajamos grafos computacionales a FPGAs, GPUs y NPUs emergentes con modelado explícito de jerarquías de memoria, flujos de datos y restricciones específicas del backend para lograr inferencia de baja latencia en dispositivos de borde. Tercero, compilación cuántica: optimización de circuitos, descomposición de puertas, enrutamiento de qubits y transpilación consciente del ruido que adaptan los circuitos cuánticos lógicos a la conectividad y perfiles de error del hardware NISQ real. Finalmente, teoría clásica de compiladores y lenguajes de programación: aprovechamos la infraestructura basada en LLVM para análisis estático y transformaciones de programas, y exploramos semánticas formales que garantizan la corrección de las optimizaciones. Este eje sustenta muchas de nuestras otras áreas de investigación — desde semiconductores de IA y computación cuántica hasta el despliegue de IA médica en dispositivos de borde."
    },
    tags: ["AI/ML Compiler", "Hardware-aware", "Quantum Compiler", "LLVM / PL"],
    directions: [
      { en: "Graph-level optimization passes, operator fusion, and autotuning in the spirit of TVM, XLA, and MLIR", ko: "TVM, XLA, MLIR의 정신을 따른 그래프 수준 최적화 패스, 연산자 융합, 자동 튜닝", ja: "TVM、XLA、MLIRの精神に倣ったグラフレベル最適化パス、演算子融合、オートチューニング", es: "Pasadas de optimización a nivel de grafo, fusión de operadores y autoajuste al estilo de TVM, XLA y MLIR" },
      { en: "Lowering and scheduling for FPGA, GPU, and NPU backends with memory hierarchy and dataflow modeling", ko: "메모리 계층 및 데이터 흐름 모델링과 함께 FPGA, GPU, NPU 백엔드를 위한 로어링 및 스케줄링", ja: "メモリ階層とデータフロー・モデリングを伴うFPGA、GPU、NPUバックエンドのローワリングとスケジューリング", es: "Reducción de nivel y planificación para backends FPGA, GPU y NPU con modelado de jerarquía de memoria y flujo de datos" },
      { en: "Circuit optimization, gate decomposition, qubit routing, and noise-aware transpilation for NISQ devices", ko: "NISQ 장치를 위한 회로 최적화, 게이트 분해, 큐비트 라우팅, 노이즈 인식 트랜스파일레이션", ja: "NISQデバイス向けの回路最適化、ゲート分解、キュービットルーティング、ノイズ対応トランスパイル", es: "Optimización de circuitos, descomposición de puertas, enrutamiento de qubits y transpilación consciente del ruido para dispositivos NISQ" },
      { en: "LLVM-based static analysis, program transformations, and formal semantics for provably safe optimizations", ko: "LLVM 기반의 정적 분석, 프로그램 변환, 그리고 안전성이 증명된 최적화를 위한 형식 의미론", ja: "LLVMベースの静的解析、プログラム変換、形式的に正当性が保証された最適化のための形式意味論", es: "Análisis estático basado en LLVM, transformaciones de programas y semánticas formales para optimizaciones demostrablemente seguras" },
    ],
  },
];
