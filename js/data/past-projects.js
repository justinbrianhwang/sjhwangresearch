...
const PAST_PROJECTS = [
  ...
  },
  {
    ...
    description: {
      en: "Detected adversarial attacks generated by generative AI models using Topological Data Analysis (TDA). Leveraged TDA to capture hidden topological features and structural anomalies that conventional detection methods cannot identify, bridging shape-based analysis with adversarial ML. Paper submitted (review pending).",
      ko: "생성형 AI 모델이 만들어낸 적대적 공격을 위상적 데이터 분석(TDA)으로 탐지. TDA를 활용해 기존 탐지 기법으로는 포착할 수 없는 숨겨진 위상적 특징과 구조적 이상을 감지함으로써, shape 기반 분석과 적대적 ML을 연결. 논문 제출 완료(심사 진행 중).",
      ja: "生成AIモデルによる敵対的攻撃を位相的データ分析(TDA)で検出。TDAを活用し従来の検出手法では捉えられない隠れた位相的特徴と構造的異常を検知。論文提出済み(査読中)。",
      es: "Detección de ataques adversariales generados por modelos de IA generativa mediante análisis topológico de datos (TDA). Aprovechamiento de TDA para capturar características topológicas ocultas y anomalías estructurales. Artículo enviado (revisión pendiente)."
    },
    year: "2026",
    outcome: { en: "Completed · Paper submitted (under review)", ko: "완료 · 논문 제출 (심사 중)", ja: "完了 · 論文提出（査読中）", es: "Completado · Artículo enviado (en revisión)" },
    teamLogos: ["assets/logos/sjjb logos.png", "assets/logos/Permillion Logos.png"],
